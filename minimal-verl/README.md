...
**情感分类本身不需要 Agentic RL。**

它单步、闭集、无信用分配——按 [25 章](../25-判别能力外置-什么时候不该用RL.md) 的判据，这类任务现在可以用一个 Schema 直接解决，不必训练。

那这个示例还有价值吗？**有，但价值在别处：**

| 这个示例能证明的 | 这个示例不能证明的 |
|---|---|
| ✅ TRL 能在本机跑通 SFT → GRPO 全链路 | ❌ RL 相对 Schema 的必要性 |
| ✅ 数据格式、训练循环、评估流程长什么样 | ❌ 多步轨迹与信用分配 |
| ✅ reward 信号如何传导、KL 如何变化 | ❌ 环境交互与工具调用 |

**换句话说：它是"工具链验证示例"，不是"Agentic RL 能力示例"。** 2 分钟跑通整条链路，这个价值是真实的——但不要把它当成"Agentic RL 的典型任务"。

如果你要的是一个**能体现 RL 不可替代性**的示例，见 [docs/04-多步任务改造方案.md](docs/04-多步任务改造方案.md)。
...
├── rl/
│   ├── reward.py                   # 0/1 严格奖励函数（情感分类任务）
│   ├── env.py                      # 多轮信息收集环境（策略类任务，待接入训练）
│   └── train_grpo.py               # trl.GRPOTrainer (FP32 + MPS)
├── eval/
│   └── evaluate.py                 # 测准确率
├── tests/
│   └── test_env.py                 # rl/env.py 单元测试（11 项，python tests/test_env.py）
├── docs/
│   ├── 01-为什么这么设计.md        # 设计决策 + 理由
│   ├── 02-数据流图.md              # 端到端数据流
│   ├── 03-常见问题.md              # FAQ
│   └── 04-多步任务改造方案.md      # 从判别任务改造为策略任务
