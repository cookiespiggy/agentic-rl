...
阶段六（时代补充）: 25    → 判别能力外置与判断模型的能力边界        📖 待学习
```
...
> **参考答案**：1. 见 1.3 节表格对比；2. B、C（需要多轮交互和工具使用的场景）。**注意 D（垃圾邮件分类）被排除的根本原因**——不只是"不需要多轮交互"，更在于它属于**判别类任务**：单步、闭集、无信用分配，因此可以用一个 Schema 直接解决，不需要训练。判别与策略的边界详见 [25 - 判别能力外置](./25-判别能力外置-什么时候不该用RL.md)；3. Policy 决定下一步动作，Memory 维护历史上下文，Tool Use 扩展 Agent 能力边界。
